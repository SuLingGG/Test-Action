...
clash_main_url=$(curl -sL https://api.github.com/repos/Dreamacro/clash/releases/latest | grep /clash-linux-$1 | sed 's/.*url\": \"//g' | sed 's/\"//g')
...
wget -qO- $clash_main_url | gunzip -c > openclash/core/clash
wget -qO- $clash_tun_url | gunzip -c > openclash/core/clash_tun
wget -qO- $clash_game_url | tar xOvz > openclash/core/clash_game
